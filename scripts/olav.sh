    ...
    if [ "${2}" = "start" ]; then
        if is_in_list "${VALID_SESSIONS}" ${3}; then
            ...
            if [ "${3}" = "description" ]; then
                ...
                exit 0
            elif [ "${3}" = "autonomy-msdwa" ]; then
                    ros2 launch olav_launch msdwa.launch.py parameters_overrides:=${HOME}/ROS/config/parameters/olav_parameters_overrides.yaml
                    exit 0
            elif [ "${3}" = "autonomy-tracker" ]; then
                ros2 launch olav_launch tracker.launch.py parameters_overrides:=${HOME}/ROS/config/parameters/olav_parameters_overrides.yaml
                exit 0
            else
